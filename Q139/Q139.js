// List three reserved words in JavaScript and create a valid use case for each.
// Reserved words in javascript are words that have a special meaning and cant be used as identifiers(e.g variable,names,function names).Understanding these can help prevent syntax errors and make your code more readable.
// Reserved word : let - used to declare a block scoped local variable
var count = 5;
// Reserved word : if - used to execute a block of code if a specified condition is true
if (count > 0) {
    console.log("Count is greator than 0.");
}
// Reserved word : return - used to exit a function and return a value from that function
function add(a, b) {
    return a + b;
}
// demonstrates valid use cases for the reserved words "let", "if", and "return".
